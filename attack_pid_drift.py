...
def monitor_attitude(mav, duration):
    """Watch attitude during/after injection; returns peak excursions."""
    start = time.time()
...
def main():
    ...
    parser.add_argument('--drift-interval', type=float, default=3.0)
    parser.add_argument('--drift-fraction', type=float, default=0.08,
                         help='Fractional gain reduction per step (0.08 = 8%%)')
    ...
    with open(gt_filename, 'w', newline='') as gt_file:
        ...
        print('Monitoring post-attack attitude behaviour (15s)...')
